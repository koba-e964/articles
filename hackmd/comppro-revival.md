...
## [AGC076](https://atcoder.jp/contests/agc076) (2025-12-28)
### よかったところ
- A は解けた
  - 実装は 10 分もかからなかった
...
### 改善できそうなところ
- A に時間がかかった
  - そもそも N^2+N-1 というのを思いつくのに時間がかかった (30min?)
  - N^2+N-1 を「検証」しようとしてしまった (20min?)
    - 数え上げなので、サンプルが合えばそれで良い
- B の 2 回やる部分を詰めきれなかった
  - 解説とちょっと違う
  - 解説のようにスッキリとした考察ができない
  - そもそも「良い考察」がなんなのか分かっていない
- ともかく全体的に**遅い**
  - 解法がなかなか頭の中で組み上がらない
    - ワーキングメモリーが狭い
    - 慣れていない
    - すでに解いた問題を復習するのにも時間がかかる
    - そもそも考えるのが面倒だと思っている
...
### パフォーマンス予測
- 実態: [208th, 2154](https://atcoder.jp/users/kobae964/history/share/agc076)
- うまく行った場合: [153rd, 2356](https://atcoder.jp/users/confeito/history/share/agc076) (30%)
- 期待値 (exp をとる): 2222

```python
>>> import math
>>> math.log2(2**(2356/400)*0.3 + 2**(2154/400)*0.7)*400
2222.347930535672
```
