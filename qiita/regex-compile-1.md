...
#### Thompson's VM
文字を一つずつ調べ、その文字まで文字列ポインタが進んでいる状態で、命令ポインタが存在し得る場所を全列挙する方式です。この計算は ε-NFA のような動的計画法[^note:dp-like-eps-nfa]ででき、計算量は、命令の個数を $n$ とすると、$O(nL)$ 時間、$O(n)$ 空間です。

[^note:dp-like-eps-nfa]: 文字を与えてどこの状態に行けるか全列挙した後、そこから ε-遷移だけで行ける頂点を全列挙するタイプの動的計画法。どちらも ε-NFA の状態数を $n$ として $O(n)$ 時間で可能。
...
# 参考文献
- [正規表現しちへんげ！](http://www.kmonos.net/wlog/115.html#_2217101203)
