...
これは木 DP によって $O(N)$ 時間で解ける。また Euler tour によって部分根付き木に区間を割り当てることにすれば、一点更新と区間和が計算できれば良いのでセグメント木や BIT により一点更新 query が $O(\log N)$ 時間になる。
...
$X$ をソートすれば良いのでこれは $O(N \log N)$ 時間で解ける。$X$ をバケットソートすることにすれば遅延セグメント木を使って一点更新 query が $O(\log N)$ 時間でできる。 ($-N \le X[i] \le N$ のものだけカウントすることに注意。)
...
最小値を求めれば良いので計算は $O(N)$ 時間ででき、セグメント木に載せれば一点更新 query は $O(\log N)$ 時間で処理できる。
